function error_ellipse(Mu,Sigma,num_GMM,data)
plot(data(:,1), data(:,2), '.');
hold on
p = zeros(num_GMM,1);
for i = 1:num_GMM
    covariance = Sigma( (i-1)*2+1:i*2 , : );
    avg = Mu(i,:);
    p(i) = one_loop(avg,covariance,i);
end
...
function p = one_loop(avg,covariance,i)
...
p = plot(r_ellipse(:,1) + X0,r_ellipse(:,2) + Y0,'-','LineWidth',2,'DisplayName',join(['component',int2str(i)]));
